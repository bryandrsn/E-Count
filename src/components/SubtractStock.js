...
export default function SubtractStock() {
    ...
    const handleSubtractStock = async () => {
        if (itemStock !== 0) {
            try {
                ...
                const stock = itemStock > currentStock ? currentStock : itemStock;
                const docRef = doc(db, "items", itemsData.id);
                await updateDoc(docRef, { stock: currentStock - stock })
                Alert.alert("Kurangi Stok Berhasil!", "Stok berhasil dikurangi.");
                resetState();
                setScanned(false);
            }
            catch (error) {
                console.error("Subtract Stock Error:", error);
                Alert.alert("Terjadi Kesalahan!", "Tidak dapat terhubung ke server. Silakan coba lagi.");
            }
        }
        else {
            Alert.alert("Tidak boleh nol!", "Silakan masukkan jumlah stok yang ingin dikurangi.");
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            {scanned && (
                ...
                <Text style={styles.textTitle}>Jumlah stok yang ingin dikurangi:</Text>
                ...
                <View style={styles.buttonContainer}>
                    <TouchableOpacity style={styles.buttonSubtract} onPress={() => handleSubtractStock()}>
                        <Text style={styles.buttonText}>Kurangi Stok</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
